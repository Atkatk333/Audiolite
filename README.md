...
- Audiolite 两列测于 Win11 22631。私有内存随运行时间上涨,其中大部分来自其他软件注入本进程的模块(输入法等),该进程共加载 67 个模块;程序自身在空闲 25 秒的两次采样中,内存、句柄、线程均无增长,180 次枚举后句柄增量 +0/+1。程序自身无任何网络导入。
...
- 排序依据的接入时间属性 `{194ef948-…},2` 未见于微软文档,实测行为与端点最近一次接入的时间一致。若微软改变其语义,影响限于菜单中设备的排列顺序,不影响切换本身,切换使用 MMDevice ID。`--list` 的排序键列显示每台实际采用的排序依据。
