...
Audiolite.exe --list          全部渲染端点(含未插入、已禁用):状态、排序键、ID、菜单名
...
有些规则纯函数测试碰不到,处理方式分两类,写清楚免得被当成"都测到了":编号、切换判定、状态文件取值、临时文件清理、横幅几何这几处已提成纯函数并用变异测试卡住(把排序键换成安装时间、去掉切换后的回读比对、去掉"上一台就是当前设备就清空记忆"、让清空记号失效、去掉临时文件的时间判据、让"放得下"恒为真——都会让测试失败);掩码语义只能在真机上按不变量校验(`test.exe` 里那两条);横幅坐标越界只在运行时写 `diag.txt`,没有断言。还有一条已知的空档:"切换成功后写 `lastId`"那行赋值,要覆盖它必须真改一次声音输出。

## 已知边界

- **接入时间属性是未公开的。** 排序用的 `{194ef948-…},2` 不在微软文档化的属性列表里。实测行为与"最近一次接入时间"一致(内置扬声器停在装机日、蓝牙耳机停在上次连接日),且已排除"上次设为默认时间"这一候选(设为默认不刷新它)。**本机 16 个端点里只有 7 个带这个属性**,取不到的那些按安装时间排——回退路径才是多数路径(`--list` 第 3 列直接标出每台走的是哪条)。微软改语义的风险存在,但影响面只是菜单顺序,不影响切换正确性——真正用于切换的是 MMDevice ID。
